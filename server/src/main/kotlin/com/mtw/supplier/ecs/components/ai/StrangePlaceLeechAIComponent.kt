...
@Serializable
class StrangePlaceLeechAIComponent: AIComponent() {
    ...
    override fun decideNextActions(encounterState: EncounterState): List<Action> {
        ...
        return if (!isInFow) {
            ...
            when ((1..100).random()) {
                in 1..50 -> listOf(WaitAction(parent, "The leech raises up, as if sniffing."))
                else -> {
                    val move = moveToPlayer(parent, parentAbsPos, playerAbsPos, encounterState)
                    if (move != null) {
                        listOf(move, TerrifyAction(parent, player, TerrorChangeStats(3, 0, 80,
                            "The leech springs towards you.")))
                    } else {
                        listOf(WaitAction(parent, "The leech raises up, as if sniffing."))
                    }
                }
            }
        } else {
            listOf(TerrifyAction(parent, player, TerrorChangeStats(10, 0, 100,
                "The leech jumps as you, but you manage to avoid it.")))
        }
    }
