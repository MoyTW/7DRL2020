    ...
    override fun decideNextActions(encounterState: EncounterState): List<Action> {
        ...
        if (!isAdjacent) {
            return when ((1..100).random()) {
                ...
                in 51..75 -> listOf(TerrifyAction(parent, player, TerrorChangeStats(2, 0, 70,
                    "He looks idly over at you. \"Hi, kid\".")))
                in 76..90 -> listOf(TerrifyAction(parent, player, TerrorChangeStats(3, 0, 75,
                    "He calls a greeting to you. You feel a creeping dread.")))
                else -> {
                    val move = moveToPlayer(parent, parentAbsPos, playerAbsPos, encounterState)
                    if (move != null) {
                        listOf(move, TerrifyAction(parent, player, TerrorChangeStats(5, 0, 80,
                            "\"Hey!\" he calls, waving, walking towards you.")))
                    } else {
                        listOf(TerrifyAction(parent, player, TerrorChangeStats(0, 0, 100,
                            "\"Hey!\" he yells, but he can't get to you.")))
                    }
                }
            }
        } else {
            return when ((1..100).random()) {
                ...
                in 26..50 -> listOf(TerrifyAction(parent, player, TerrorChangeStats(3, 0, 80,
                    "He converses, politely.")))
                in 51..75 -> listOf(TerrifyAction(parent, player, TerrorChangeStats(5, 0, 90,
                    "He is standing far too close as he speaks.")))
                in 76..90 -> listOf(TerrifyAction(parent, player, TerrorChangeStats(8, 0, 100,
                    "You can't breathe.")))
